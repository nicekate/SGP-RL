...
def tifa_score_benchmark(vqa_model_name, question_answer_path, id2img_path, id2img_parent_path=None):
    ...
    if id2img_parent_path is None: id2img_parent_path = os.path.dirname(id2img_path)
    ...
    for question_answer_pair in tqdm(question_answer_pairs):
        ...
        if str(caption_id) not in caption_id2img_fn:
            print(f"Caption id {caption_id} not found in {id2img_path}!")
            continue
        img_fns = caption_id2img_fn[str(caption_id)]
        free_form_answer_list = []
        mc_answer_list = []
        score_list = []
        if img_fns == []: 
            print(f"Image corresponding to {caption_id} not found in {id2img_path}!")
            continue
        for img_fn in img_fns:
            ...
            print(f"get_answer time: {time.time() - start_time}")
            free_form_answer, mc_answer = vqa_answer["free_form_answer"], vqa_answer["multiple_choice_answer"]
            free_form_answer_list.append(free_form_answer)
            mc_answer_list.append(mc_answer)
            score = int(mc_answer == question_answer_pair['answer'])
            score_list.append(score)
            print(f"Infer single image time: {time.time() - start_time}")

        question_logs[caption_id][question_answer_pair['question']]['free_form_vqa'] = free_form_answer_list
        question_logs[caption_id][question_answer_pair['question']]['multiple_choice_vqa'] = mc_answer_list        # compute multiple choice score
        question_logs[caption_id][question_answer_pair['question']]['score_list'] = score_list
        question_logs[caption_id][question_answer_pair['question']]['scores'] = mean(score_list)
    ...
    # accuracy by types
    print("Accuracy by types:")
    for type_, accuracy in result_dict["accuracy_by_type"].items():
        print(f"{type_}: {accuracy:.4f}")
