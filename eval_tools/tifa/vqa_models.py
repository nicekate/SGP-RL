...
class MPLUG:
    def __init__(self, ckpt='damo/mplug_visual-question-answering_coco_large_en'):
        self.pipeline_vqa = pipeline(Tasks.visual_question_answering, model=ckpt, device='gpu')
        # self.pipeline_vqa.model = self.pipeline_vqa.model.to('cuda')
    ...
    def vqa(self, image, question):
        if isinstance(image, list):
            ...
            input_vqa  = [{'image': image[i], 'question': question[i]} for i in range(batch_size)]
...
class VQAModel:
    ...
    def vqa(self, image, question, choices=[]):
        ...
        with torch.no_grad():
            ...
            if (len(choices) != 0) and (self.model_name.startswith("blip2")):
                # print(f"VQA time: {time.time() - start_time}")
                result = self.model.vqa(image, question, choices)
            else:
                result = self.model.vqa(image, question)
        print(f"VQA time: {time.time() - start_time}")
        return result
    ...
    def multiple_choice_vqa(self, image, question, choices):
        ...
        if free_form_answer not in choices:
            ...
            print(f"MC time: {time.time() - start_time}")
