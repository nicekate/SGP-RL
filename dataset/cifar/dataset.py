...
class CifarDataset:
    ...
    def load_dataset(
        ...
        **kwargs
        ...
        CifarDataset.label_names = dataset["train"].features["fine_label"].names
        dataset = dataset.map(CifarDataset.process_example)
    ...
    def process_example(example: Dict[str, Any]) -> Dict[str, Any]:
        ...
        text_label = CifarDataset.label_names[example["fine_label"]]
        return {
            "prompt": [
                {"role": "system", "content": SYSTEM_PROMPT},
                {"role": "user", "content": f"Please write SVG code for generating the image corresponding to the following description: {text_label}"},
            ],
            "solution": text_label,
        }
